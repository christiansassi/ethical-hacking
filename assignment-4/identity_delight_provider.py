...
The RSA algorithm employed employs a 1024-bit key and utilizes the default public exponent of 65537.
...
def get_n(r: pwn.tubes.remote.remote):
    ...
    # However, due to encountering "Got EOF while reading in interactive" with pwn (likely due to a buffer overflow issue), this solution only utilizes two values for 'm'. 
    ...
    while len(messages) < 2:
        ...
        messages.append(pow(c, 2) - cp)
    
    # Calculate every combination and then return the n with the highest frequency
    n = [GCD(combination[0], combination[1]) for combination in list(combinations(messages, 2))]
...
def malleability_attack(r: pwn.tubes.remote.remote, n: int, flag: int):
    ...
    # Just to demonstrate that malleability works with any random g
